...
function __glab_perform_completion
    ...
    if not type -q "$args[1]"
        # This can happen when "complete --do-complete glab" is called when running this script.
        __glab_debug "Cannot find $args[1]. No completions."
        return
    end
...
function __glab_prepare_completions
    ...
    if not set --query __glab_comp_commandLine
        ...
        set __glab_comp_commandLine (commandline -c)
    end
...
# The call to __glab_prepare_completions will setup both __glab_comp_results and __glab_comp_do_file_comp.
